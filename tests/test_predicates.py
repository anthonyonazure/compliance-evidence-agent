...
def test_mfa_for_all_users_fails_when_only_guests():
    ev = {
        "entra_audit": {
            "policies": [_policy(user_scope_includes=["GuestsOrExternalUsers"])]
        }
    }
    ok, _ = P.has_mfa_for_all_users(ev, {})
    assert not ok
...
def test_mfa_for_admins_passes_when_admin_targeted():
    ev = {
        "entra_audit": {
            "policies": [_policy(user_scope_includes=["Global Administrator"])]
        }
    }
...
def test_blocks_legacy_auth_passes_when_block_policy_present():
    ev = {
        "entra_audit": {
            "policies": [
                _policy(display_name="Block legacy auth", grant_controls=["block"])
            ]
        }
    }
...
def test_global_admin_count_too_many():
    members = [
        {"role": "Global Administrator", "member_upn": f"u{i}@x"} for i in range(6)
    ]
...
def test_subscription_diagnostics_configured():
    ok, _ = P.subscription_diagnostics_configured(
        {"azure_resource": {"diagnostic_settings": [{"id": "x"}]}}, {}
    )
    assert ok
    ok, _ = P.subscription_diagnostics_configured(
        {"azure_resource": {"diagnostic_settings": []}}, {}
    )
    assert not ok
...
def test_storage_tls_minimum_12_pass_and_fail():
    pass_facts = [
        _fact("microsoft.storage/storageaccounts", minimumTlsVersion="TLS1_2")
    ]
    fail_facts = [
        _fact("microsoft.storage/storageaccounts", minimumTlsVersion="TLS1_0")
    ]
    assert P.storage_tls_minimum_12({"azure_resource": {"facts": pass_facts}}, {})[0]
    assert not P.storage_tls_minimum_12({"azure_resource": {"facts": fail_facts}}, {})[
        0
    ]
...
def test_keyvault_purge_protection_enabled():
    ...
    assert P.keyvault_purge_protection_enabled(
        {"azure_resource": {"facts": facts}}, {}
    )[0]
    facts = [_fact("microsoft.keyvault/vaults", enablePurgeProtection=False)]
    assert not P.keyvault_purge_protection_enabled(
        {"azure_resource": {"facts": facts}}, {}
    )[0]
